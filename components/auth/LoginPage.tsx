...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onNavigateToRegister, onBack, language }) => {
  ...
  const handleSocialLogin = async (provider: 'Google' | 'GitHub') => {
      setIsLoading(true);
      // Social login simulation logic remains inside component or can be moved to API service
      setTimeout(() => {
          const mockUser: User = {
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white dark:bg-dark-900 text-slate-500">{t.orSignIn}</span>
          </div>
        </div>
        ...
        <p className="text-center text-sm text-slate-600 dark:text-slate-400">
          {t.dontHaveAccount}{' '}
          <button onClick={onNavigateToRegister} className="font-semibold text-brand-600 hover:text-brand-500 hover:underline">
            {t.createAccount}
          </button>
        </p>
      </form>
